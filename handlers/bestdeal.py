    ...
    try:
        ...
        for loc in town_loc:
            key_city = InlineKeyboardButton(
                loc, callback_data="city_dist" + town_loc[loc]
            )
            key.add(key_city)
...
def city_name_dist(message):
    chat_id = message.chat.id
    if chat_id not in user_data:
        user_data[chat_id] = {}
    user_data[chat_id]["city_input"] = message.text
    search_city_dist(message)


@bot.callback_query_handler(func=lambda call: call.data.startswith("city_dist"))
def callback_arrival_dist(call):
    ...
    calendar, step = DetailedTelegramCalendar(
        calendar_id="Adist", min_date=date.today()
    ).build()
...
@bot.callback_query_handler(
    func=lambda call: DetailedTelegramCalendar.func(calendar_id="Adist")(call)
)
def handle_arrival_date_selection_dist(call):
    chat_id = call.message.chat.id
    try:
        ...
        if not result and key:
            ...
            calendar, step = DetailedTelegramCalendar(
                calendar_id="Ddist", min_date=min_date
            ).build()
...
@bot.callback_query_handler(
    func=lambda call: DetailedTelegramCalendar.func(calendar_id="Ddist")(call)
)
def handle_departure_date_selection_dist(call):
    chat_id = call.message.chat.id
    try:
        min_date = datetime.strptime(
            user_data[chat_id]["arrival_date"], "%Y-%m-%d"
        ...
        if not result and key:
            ...
            count_hotel_dist(call.message)
    except Exception:
        bot.send_message(chat_id, ERROR_UN)


def count_hotel_dist(message):
    chat_id = message.chat.id
    ...
    bot.register_next_step_handler(msg, process_hotel_count_dist)


def process_hotel_count_dist(message):
    chat_id = message.chat.id
    try:
        hotel_count = int(message.text)
        user_data[chat_id]["hotel_count"] = hotel_count
        search_hotel_dist(message)
    except ValueError:
...
def search_hotel_dist(message):
    ...
    if dest_id and arrival_date and departure_date:
        QUERY_HOTEL_DIST["dest_ids"] = dest_id
        QUERY_HOTEL_DIST["arrival_date"] = arrival_date
        QUERY_HOTEL_DIST["departure_date"] = departure_date

        try:
            response = requests.get(URL_HOTEL, headers=HEADERS, params=QUERY_HOTEL_DIST)
            response.raise_for_status()
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("photo_dist"))
def handle_photo_dist(call):
    ...
    try:
        ...
        response.raise_for_status()
        photo = response.json()
        for ph in range(min(5, len(photo["data"][QUERY_PHOTO["hotel_ids"]]))):
            photos.append(
